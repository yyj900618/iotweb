    ...
    <el-row class="container">
        ...
        <el-col :span="24" class="main">
            <aside :class="collapsed?'menu-collapsed':'menu-expanded'">
                <!--导航菜单-->
                <el-menu background-color="#407db9" text-color="#fff" active-text-color="#ffd04b" v-bind:style=" collapsed ? {} : { 'width': '230px' } " :default-active="$route.path" class="el-menu-vertical-demo" unique-opened router v-show="!collapsed">
                   <!--**************************************** -->
                   <!-- 一级菜单 -->
                    <template v-for="(item,index) in menuData" v-if="!item.hidden">
                        <el-submenu :index="index+''" v-if="!item.leaf" v-hasPermission="item.meta.permission">
                            
                            <template slot="title">
                                <i :class="item.iconCls"></i>{{item.name}}
                            </template>
                            
                            <el-menu-item v-for="child in item.children" :index="child.path" :key="child.path" v-if="!child.hidden" v-hasPermission="child.meta.permission">
                                {{child.name}}
                            </el-menu-item>
...
<script>
    ...
    export default {
        ...
        mounted: function() {
            this.initMenu();
            this.sysUserName=sessionStorage.getItem("name")
